...
# ----------------------------------------------------------------------------------------------------------------------------------- PREP
    ...
    def map_components(self): # strings components together in a path to exit
        ...
        # define all components
            self.components = { 'n-wall': Component([0, 0, 0], {'left':'w-wall', 'right':'e-wall'}, {}),
                                ...
                                'ladder-conversion': Component([0, 0, 1], {}, {})
                                }

            self.components_to_place = ['cipher-chest', 'cipher-painting', 'door', 'bed', 'simon-says', 'ladder-conversion']
            ...
            if 'ladder-conversion' in self.components_to_place:
                self.setup_ladder_conversion()
    ...
    def setup_ladder_conversion(self):

        self.ladder_conversion_difficulty = 'easy'

        if self.ladder_conversion_difficulty == 'easy':
            self.ladder_choice = random.choice([1,2]) # selects painting 1 or 2
            numbers = [1, 2, 3, 4, 5, 6]
            random.shuffle(numbers)
            self.ladder_numbers_order = numbers
            if self.ladder_choice == 1:
                self.ladder_map = {1:1, 2:4, 3:3, 4:5, 5:6, 6:2}
            elif self.ladder_choice == 2:
                self.ladder_map = {1:4, 2:2, 3:1, 4:3, 5:5, 6:6}
            colors = ['yellow', 'red', 'orange', 'purple', 'red', 'pink', 'blue', 'green']
            random.shuffle(colors)
            self.ladder_colors_order = colors[:6]
...
# -------------------------------------------------------------------------------------------------------------------- EXECUTE
    ...
    def main(self):
        ...
        while True: # main game loop
            ...
            self.update_display() # update seconds time?
...
# --------------------------------------------------------------------------------------------------------- DRAW
    ...
    def update_display(self):
        ...
        elif self.state == 'ladder-conversion':
            self.background = pygame.image.load('pics/ladder-conversion-full-{}.png'.format(self.ladder_choice))
            for i, ladder_number in enumerate(self.ladder_numbers_order):
                number = pygame.image.load('pics/number-{}-medium.png'.format(ladder_number))
                self.background.blit(number, (112 + i * 127, 3))
            for i, ladder_color in enumerate(self.ladder_colors_order):
                color = pygame.image.load('pics/{}-medium.png'.format(ladder_color))
